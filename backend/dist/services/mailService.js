...
class MailService {
    constructor() {
        ...
        // Pre-load templates
        const resolvedTemplatesDir = fs_1.default.existsSync(templatesDir) ? templatesDir : legacyTemplatesDir;
        this.templates = {
            orderConfirmation: fs_1.default.readFileSync(path_1.default.join(resolvedTemplatesDir, 'orderConfirmation.html'), 'utf8'),
            ownerNotification: fs_1.default.readFileSync(path_1.default.join(resolvedTemplatesDir, 'ownerNotification.html'), 'utf8')
        };
    }
    /**
        ...
        try {
            ...
            if (upn) {
                ...
                if (!fs_1.default.existsSync(invoicePath)) {
                    logger_1.default.error(`Invoice file not found: ${invoicePath}`);
            ...
            logger_1.default.info(`Order confirmation email sent successfully for order #${order.id}`, {
            ...
            logger_1.default.error('Error sending order confirmation email:', error);
            throw error;
        }
    }
    /**
    ...
    async sendOwnerOrderNotification(order, glsLabelPath = null) {
        try {
            logger_1.default.info(`Preparing to send owner notification for order #${order.id}, GLS label: ${glsLabelPath ? 'yes' : 'no'}`);
            // Validate GLS label exists if path provided
            if (glsLabelPath && !fs_1.default.existsSync(glsLabelPath)) {
                logger_1.default.warn(`GLS label file not found: ${glsLabelPath}`);
            ...
            const orderItemsText = order.orderItems.map(item => `${item.productName}\n  Količina: ${item.quantity}\n  Cena: ${item.price.toFixed(2)} EUR\n  Skupaj: ${(item.price * item.quantity).toFixed(2)} EUR`).join('\n\n') + `\n\nVmesni seštevek: ${subtotalOwner.toFixed(2)} EUR\nDostava: ${SHIPPING_FEE.toFixed(2)} EUR`;
            // Replace variables in template with escaped user data
            ...
            if (!ownerEmail) {
                logger_1.default.warn('OWNER_EMAIL not configured in environment variables');
                return {
                    success: false,
            ...
            const info = await this.transporter.sendMail(mailOptions);
            logger_1.default.info(`Owner notification email sent successfully for order #${order.id}${glsLabelPath ? ' with GLS label' : ''}`, {
                messageId: info.messageId,
            ...
            logger_1.default.error('Error sending owner notification email:', error);
            throw error;
        }
    }
}
